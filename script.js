...
for (let i = 0; i < heartParticleCount; i++) {
    ...
    heartParticles.push({
        ...
        tx: (heartTarget.x * internalDist) + centerX, ty: (heartTarget.y * internalDist) + centerY,
        origTx: (heartTarget.x * internalDist) + centerX, origTy: (heartTarget.y * internalDist) + centerY,
        ...
        heartColor: finalColor, alpha: Math.random() * 0.5 + 0.3,
        wobble: Math.random() * 100, wobbleSpeed: Math.random() * 0.04 + 0.01,
        isStray: i < strayCount,
        orbitRadius: Math.random() * (canvas.width * 0.4) + 10,
        orbitAngle: Math.random() * Math.PI * 2, orbitSpeed: Math.random() * 0.06 + 0.03,
        burstProgress: 0, burstSpeed: Math.random() * 0.04 + 0.03,
        strayVx: Math.cos(angle) * (Math.random() * 25 + 20), 
        strayVy: Math.sin(angle) * (Math.random() * 25 + 20),
        vortexAngleX: Math.random() * Math.PI * 2, vortexAngleY: Math.random() * Math.PI * 2,
        vortexSpeedX: (Math.random() - 0.5) * 0.15, vortexSpeedY: (Math.random() - 0.5) * 0.15,
...
function parseQR() {
    ...
    if (qrPositions.length > 0) {
        ...
        const qrScale = isMobile ? 0.0022 : 0.0022;
