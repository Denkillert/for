...
function getHeartPoint(t) {
    ...
    // ИСПРАВЛЕНО: добавлены знаки умножения 2 * t, 3 * t
...
// === ИСПРАВЛЕННЫЙ И УНИВЕРСАЛЬНЫЙ ОБРАБОТЧИК НАЖАТИЯ ===
function triggerSurprise(e) {
    // Предотвращаем стандартное поведение (зум/скролл) при тапе на мобильных
...
// Вешаем оба события для максимальной совместимости с iOS и Android
qrBtn.addEventListener("click", triggerSurprise);
qrBtn.addEventListener("touchstart", triggerSurprise, { passive: false });

// ИСПРАВЛЕНО: убраны лишние пробелы и сломанные стрелочные функции
        ...
        if (mode === "supernova") {
            ...
            if (p.isTextPointer) {
                ...
                if (distance < 6) {
                    ...
                    if (spans[p.letterIndex]) {
                        spans[p.letterIndex].classList.add("visible");
                    }
                    p.alpha -= 0.05;
                    if (p.alpha < 0) p.alpha = 0;
                }
